      ...
      onFile: function (file, content, h) {
        ...
        heic2any({ blob, toType: 'image/jpeg', quality: 0.9 })
          .then(result => {
             ...
             h.render(`
               <div class="flex flex-col h-[85vh] border border-surface-200 rounded-xl overflow-hidden bg-surface-100 shadow-sm font-sans">
                 ...
                 <div class="flex-1 overflow-auto p-12 flex justify-center items-center">
                    <img id="heic-preview" src="${url}" class="max-w-full h-auto shadow-2xl rounded-lg bg-white transition-transform duration-200" style="transform: scale(1)" />
                 </div>
               </div>
             `);
             ...
             document.getElementById('btn-zoom-out').onclick = () => { if(scale > 0.4) scale -= 0.2; update(); };
             document.getElementById('btn-dl').onclick = () => h.download(`${file.name}.jpg`, resBlob, 'image/jpeg');
             
             const update = () => { document.getElementById('heic-preview').style.transform = `scale(${scale})`; };
          ...
          .catch(err => {
             h.render(`
               <div class="p-12 text-center text-surface-400">
                  ...
                  <p>Unable to convert this HEIC image. The file may be empty or corrupted.</p>
               </div>
             `);
          });
      }
