 ...
 * Uses OmniTool SDK. Parses mailbox files into individual messages.
 */
(function () {
  'use strict';
  ...
  window.initTool = function (toolConfig, mountEl) {
    OmniTool.create(mountEl, toolConfig, {
      accept: '.mbox',
      binary: false,
      infoHtml: '<strong>MBOX Toolkit:</strong> Professional mailbox viewer with message extraction and individual email inspection.',
      ...
      onFile: function (file, content, h) {
        ...
        const renderList = (filtered) => `
          <div class="divide-y divide-surface-100">
            ${filtered.map((m, i) => `
              <div class="msg-item p-4 hover:bg-surface-50 cursor-pointer transition-colors flex flex-col gap-1" data-index="${i}">
            ...
            <div class="w-80 shrink-0 border-r border-surface-200 flex flex-col bg-surface-50/30">
               <div class="p-4 border-b border-surface-200 bg-white">
                  <div class="flex items-center justify-between mb-2">
                     <span class="text-[10px] font-bold text-surface-400 uppercase tracking-widest">Inbox</span>
        ...
        const openMsg = (idx) => {
           ...
           view.innerHTML = `
             ...
             <div class="p-4 border-t border-surface-100 bg-surface-50 flex justify-end">
                <button id="btn-dl-msg" class="px-3 py-1.5 bg-white border border-surface-200 rounded text-xs font-bold hover:bg-surface-100">📥 Export .eml</button>
             </div>
           `;
