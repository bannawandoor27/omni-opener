  ...
  window.initTool = function (toolConfig, mountEl) {
    OmniTool.create(mountEl, toolConfig, {
      ...
      onFile: async function (file, content, h) {
        h.showLoading('Analyzing ISO image...');
        ...
        if (buffer.byteLength >= 32774) {
          const magic = new TextDecoder().decode(buffer.slice(32769, 32774));
          if (magic === 'CD001') {
            isISO = true;
          }
        }
          ...
          <div class="p-6 space-y-6 font-sans">
            <div class="flex items-center justify-between border-b border-surface-200 pb-4">
              <div>
                ...
                <p class="text-sm text-surface-500">${(file.size / (1024 * 1024)).toFixed(2)} MB • Optical Disk Image</p>
              </div>
              ...
              <div class="bg-surface-50 rounded-xl p-4 border border-surface-200">
                ...
                ${isISO ? `
                  <div class="text-sm text-surface-600">
                    <p>Verified <strong>ISO 9660</strong> optical disk image.</p>
                    <p class="mt-2 text-xs">Primary Volume Descriptor signature 'CD001' detected at offset 0x8001.</p>
                  </div>
                ` : `
                  <div class="text-sm text-surface-400 italic">No ISO 9660 'CD001' signature detected at standard offset.</div>
                `}
              </div>
            </div>
