  ...
  window.initTool = function (toolConfig, mountEl) {
    OmniTool.create(mountEl, toolConfig, {
      ...
      onFile: async function (file, content, h) {
        ...
        // 3. Snap Analysis (SquashFS)
        let isSquashFS = false;
          ...
          <div class="p-6 space-y-6 font-sans">
            <div class="flex items-center justify-between border-b border-surface-200 pb-4">
              <div>
                ...
                <p class="text-sm text-surface-500">${(file.size / 1024).toFixed(2)} KB • Snap Package</p>
              </div>
              ...
              <div class="bg-surface-50 rounded-xl p-4 border border-surface-200">
                ...
                ${isSquashFS ? `
                  <div class="text-sm text-surface-600">
                    <p>Verified <strong>SquashFS</strong> filesystem (standard for .snap files).</p>
                    <p class="mt-2 text-xs">Snap packages are compressed SquashFS images containing applications and their dependencies.</p>
                  </div>
                ` : `
                  <div class="text-sm text-surface-400 italic">No SquashFS signature ('hsqs') detected.</div>
                `}
              </div>
            </div>
