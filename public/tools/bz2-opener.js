...
(function () {
  ...
  /**
   * Generate a hex dump for binary data.
   */
  ...
  window.initTool = function (toolConfig, mountEl) {
    OmniTool.create(mountEl, toolConfig, {
      ...
      onFile: function (file, content, h) {
        ...
        if (state.previewUrl) {
          ...
          h.setState('previewUrl', null);
        }

        return h.loadScript('https://cdn.jsdelivr.net/gh/antimatter15/bzip2.js/bzip2.js')
          .then(function () {
            if (typeof bzip2 === 'undefined') throw new Error('BZip2 library failed to initialize.');
        ...
        },
        {
          ...
          onClick: function (h, btn) {
            ...
            if (state.text) {
              ...
              h.copyToClipboard(state.hexDump, btn);
            }
          }
        }
      ]
    ...
    /**
     * Parse and render the BZip2 file.
     */
    function processFile(file, content, h) {
      var uint8 = new Uint8Array(content);

      // Verify BZip2 Signature "BZh"
      if (uint8[0] !== 0x42 || uint8[1] !== 0x5A || uint8[2] !== 0x68) {
        throw new Error('Invalid BZip2 signature (expected "BZh"). Is this a valid .bz2 file?');
      }
      ...
      try {
        ...
        // Calculate SHA-256 of the decompressed content
        return crypto.subtle.digest('SHA-256', decompressed).then(function (hashBuffer) {
          ...
          var text = isProbablyText ? new TextDecoder().decode(decompressed) : null;
          var hexDump = generateHexDump(decompressed.slice(0, 4096));
          ...
          if (isImage) {
            ...
            previewHtml = '<div class="mt-8">' +
                '<h3 class="font-bold text-surface-800 mb-4 tracking-tight">Visual Preview</h3>' +
            ...
            previewHtml = '<div class="mt-8">' +
                '<div class="flex items-center justify-between mb-3">' +
                  '<h3 class="font-bold text-surface-800 tracking-tight">Text Content Preview</h3>' +
                ...
                '</div>' +
                '<pre class="p-4 text-xs font-mono bg-gray-900 text-gray-100 rounded-xl overflow-auto max-h-[400px] leading-relaxed border border-gray-800 shadow-inner">' + esc(text.slice(0, 50000)) + (text.length > 50000 ? '\n\n... [Truncated for Performance]' : '') + '</pre>' +
              '</div>';
          } else {
            previewHtml = '<div class="mt-8">' +
                '<div class="flex items-center justify-between mb-3">' +
                  '<h3 class="font-bold text-surface-800 tracking-tight">Binary Preview (Hex Dump)</h3>' +
                ...
                '</div>' +
                '<pre class="p-4 text-[11px] font-mono bg-white text-surface-700 rounded-xl border border-surface-200 overflow-auto max-h-[400px] leading-tight shadow-inner">' + esc(hexDump) + (decompressed.length > 4096 ? '\n\n... [Truncated for Performance]' : '') + '</pre>' +
              '</div>';
          }
                  ...
                  '<div class="bg-surface-50 rounded-3xl p-6 border border-surface-200 shadow-sm">' +
                    ...
                    '<div>' +
                      ...
                      '<div class="bg-white border border-surface-200 rounded-xl p-3 flex items-center gap-2">' +
                        ...
                        '<button id="copy-hash-inner" class="p-2 hover:bg-surface-100 rounded-lg text-surface-400 hover:text-brand-600 transition-colors" title="Copy Hash">' +
        ...
        throw new Error('BZip2 Decompression Failed: ' + err.message);
      }
    }
  };
})();
