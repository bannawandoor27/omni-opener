...
(function () {
  ...
  /**
   * Helper: Generate a hex dump for binary data.
   */
  ...
  window.initTool = function (toolConfig, mountEl) {
    OmniTool.create(mountEl, toolConfig, {
      ...
      onFile: function (file, content, h) {
        ...
        // Ensure script is loaded, then process
        return h.loadScript('https://cdn.jsdelivr.net/gh/antimatter15/bzip2.js/bzip2.js')
          .then(function () {
    ...
    function processFile(file, content, h) {
      try {
        ...
        const bitstream = bzip2.array(uint8);
        const decodedString = bzip2.simple(bitstream);
        
        // Convert "binary string" output to Uint8Array
        const decompressed = new Uint8Array(decodedString.length);
        ...
        return crypto.subtle.digest('SHA-256', content).then(function (hashBuffer) {
          ...
          let previewHtml = '';
          const isImage = /\.(png|jpe?g|gif|webp|svg)$/i.test(originalName);
          ...
          if (isImage) {
            ...
            // Heuristic to check if text
            const sample = decompressed.slice(0, 1024);
            const isProbablyText = Array.from(sample).every(b => b === 10 || b === 13 || b === 9 || (b >= 32 && b <= 126));
            
            if (isProbablyText) {
              const text = new TextDecoder().decode(decompressed.slice(0, 30000));
              previewHtml = `
                <div class="mt-8">
              ...
              const hexDump = generateHexDump(decompressed.slice(0, 2048));
              previewHtml = `
                <div class="mt-8">
                  <div class="flex items-center justify-between mb-3">
          ...
          // Event Bindings
          const dlBtn = document.getElementById('main-dl-btn');
          if (dlBtn) dlBtn.onclick = function () { h.download(originalName, decompressed); };
          
          const copyBtn = document.getElementById('copy-hash-inner');
          if (copyBtn) copyBtn.onclick = function (e) { h.copyToClipboard(hashHex, e.currentTarget); };
        });
      } catch (err) {
        console.error(err);
        h.showError('Extraction Error', err.message || 'The BZip2 archive could not be processed.');
      }
    }
  };
})();
