  ...
  window.initTool = function (toolConfig, mountEl) {
    OmniTool.create(mountEl, toolConfig, {
      ...
      onFile: async function (file, content, h) {
        h.showLoading('Analyzing Ruby Gem...');
        ...
        if (buffer.byteLength >= 262) {
          const magic = new TextDecoder().decode(buffer.slice(257, 262));
          if (magic === 'ustar') {
            isTar = true;
          }
        }
          ...
          <div class="p-6 space-y-6 font-sans">
            <div class="flex items-center justify-between border-b border-surface-200 pb-4">
              <div>
                ...
                <p class="text-sm text-surface-500">${(file.size / 1024).toFixed(2)} KB • Ruby Gem Package</p>
              </div>
              ...
              <div class="bg-surface-50 rounded-xl p-4 border border-surface-200">
                ...
                ${isTar ? `
                  <div class="text-sm text-surface-600">
                    <p>Verified <strong>POSIX Tar</strong> archive (standard for .gem files).</p>
                    <p class="mt-2 text-xs">Ruby Gems are Tar archives typically containing metadata (YAML/Gzip) and the library source.</p>
                  </div>
                ` : `
                  <div class="text-sm text-surface-400 italic">No 'ustar' Tar signature detected at standard offset.</div>
                `}
              </div>
            </div>
