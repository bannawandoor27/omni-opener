(function() {
  ...
  window.initTool = function(toolConfig, mountEl) {
    ...
    OmniTool.create(mountEl, toolConfig, {
      accept: '.3mf',
      dropLabel: 'Drop a .3mf file here',
      binary: true,
      onInit: function(h) {
        // Load Three.js 0.147.0 for non-module loader support
        h.loadScript('https://cdn.jsdelivr.net/npm/three@0.147.0/build/three.min.js', () => {
          // JSZip is a mandatory dependency for 3MFLoader
        ...
        if (file.size > 30 * 1024 * 1024) {
          h.render(`
            <div class="p-12 text-center">
              ...
              <p class="text-sm text-surface-500 mb-8 mx-auto max-w-sm">This 3MF file is ${formatSize(file.size)}. Parsing large 3D packages may take some time.</p>
      ...
      infoHtml: '<strong>Privacy:</strong> 100% client-side. 3MF parsing and rendering happen entirely in your browser.'
    });
    ...
    function processFile(file, content, h) {
      ...
      // Ensure JSZip is where ThreeMFLoader expects it (global scope usually works, but some versions check specific places)
      if (typeof window.JSZip === 'undefined' && typeof JSZip !== 'undefined') {
        window.JSZip = JSZip;
      }

      h.showLoading('Parsing 3MF package...');
      try {
        const loader = new THREE.ThreeMFLoader();
        // 3MFLoader.parse expects an ArrayBuffer
        const object = loader.parse(content);
        if (!object) throw new Error('Parsed object is empty');
        renderViewer(object, file, h);
      } catch (err) {
        h.showError('Could not parse 3MF file', err.message || 'The file format might be invalid or corrupted.');
      }
    }
    ...
    function renderViewer(object, file, h) {
      ...
      // Enable shadow map if needed, but 3MF models often don't have lights defined
      ...
      // Model Positioning - 3MF often has its own scale/rotation
      // We'll center it based on its bounding box
      object.position.sub(center);
      scene.add(object);

      // Normalize scale if it's too small or too huge (optional, but good for UX)
        ...
        object.traverse(n => { 
          if (n.isMesh && n.material) {
            if (Array.isArray(n.material)) {
              n.material.forEach(m => m.wireframe = e.target.checked);
            } else {
