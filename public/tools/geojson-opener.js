...
(function () {
  ...
  window.initTool = function (toolConfig, mountEl) {
    ...
    OmniTool.create(mountEl, toolConfig, {
      accept: '.geojson,.json',
      binary: false,
      infoHtml: '<strong>GeoJSON Toolkit:</strong> Interactive map viewer with feature inspection and style switching.',
            ...
            <div class="flex flex-col h-[85vh] border border-surface-200 rounded-xl overflow-hidden bg-white shadow-sm">
              <div class="shrink-0 bg-surface-50 border-b border-surface-200 px-4 py-2 flex items-center justify-between">
                 ...
                 <div class="flex gap-2">
                    <button id="btn-fit" class="px-2 py-1 bg-white border border-surface-200 rounded text-[10px] font-bold">🎯 Fit Bounds</button>
                 </div>
              </div>
          ...
          const geojsonLayer = L.geoJSON(data, {
             ...
             onEachFeature: (feature, layer) => {
                layer.on('click', (e) => {
                   ...
                   contentEl.innerHTML = Object.entries(feature.properties || {}).map(([k, v]) => `
