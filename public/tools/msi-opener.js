  ...
  window.initTool = function (toolConfig, mountEl) {
    OmniTool.create(mountEl, toolConfig, {
      ...
      onFile: async function (file, content, h) {
        h.showLoading('Analyzing MSI package...');
          ...
          <div class="p-6 space-y-6 font-sans">
            <div class="flex items-center justify-between border-b border-surface-200 pb-4">
              <div>
                ...
                <p class="text-sm text-surface-500">${(file.size / 1024).toFixed(2)} KB • Windows Installer Package</p>
              </div>
              ...
              <div class="bg-surface-50 rounded-xl p-4 border border-surface-200">
                ...
                ${isCFBF ? `
                  <div class="text-sm text-surface-600">
                    <p>Verified <strong>Compound File Binary Format</strong> (CFBF).</p>
                    <p class="mt-2 text-xs">This format is used by Windows Installer (MSI) and older Microsoft Office documents.</p>
                  </div>
                ` : `
                  <div class="flex items-center justify-center h-24 text-surface-400 italic">Not a standard CFBF/MSI file</div>
                `}
              </div>
            </div>
