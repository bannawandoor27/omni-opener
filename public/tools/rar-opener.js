      ...
      onFile: async function (file, content, h) {
        ...
        if (view.length >= 7) {
          if (view[0] === 0x52 && view[1] === 0x61 && view[2] === 0x72 && view[3] === 0x21 && view[4] === 0x1A && view[5] === 0x07) {
            rarInfo.isValid = true;
            rarInfo.version = view[6] === 0x00 ? '5.0+' : (view[6] === 0x01 ? '4.x' : 'Unknown');
          }
        }
          ...
          <div class="p-6 space-y-6 font-sans">
            <div class="flex items-center justify-between border-b border-surface-200 pb-4">
              <div>
                ...
                <p class="text-sm text-surface-500">${(file.size / 1024).toFixed(2)} KB • RAR Archive</p>
              </div>
              ...
              <div class="bg-surface-50 rounded-xl p-4 border border-surface-200">
                ...
                ${rarInfo.isValid ? `
                  <div class="text-sm text-surface-600">
                    <p>Verified <strong>RAR</strong> archive format.</p>
                    <p class="mt-2 text-xs">Version: <span class="font-bold">${rarInfo.version}</span></p>
                  </div>
                ` : `
                  <div class="text-sm text-surface-400 italic">No RAR signature detected.</div>
                `}
              </div>
            </div>
