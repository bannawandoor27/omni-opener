  ...
  window.initTool = function (toolConfig, mountEl) {
    OmniTool.create(mountEl, toolConfig, {
      ...
      onFile: async function (file, content, h) {
        h.showLoading('Analyzing DLL file...');
          ...
          if (mzSignature === 0x5A4D) { // 'MZ'
            ...
            if (buffer.byteLength >= peOffset + 24) {
              ...
              if (peSignature === 0x00004550) { // 'PE\0\0'
                ...
                peInfo.timestamp = new Date(timestamp * 1000).toISOString();
          ...
          <div class="p-6 space-y-6 font-sans">
            <div class="flex items-center justify-between border-b border-surface-200 pb-4">
              <div>
                ...
                <p class="text-sm text-surface-500">${(file.size / 1024).toFixed(2)} KB • ${file.type || 'application/x-msdownload'}</p>
              </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <!-- PE Header Card -->
              <div class="bg-surface-50 rounded-xl p-4 border border-surface-200">
                <h4 class="text-sm font-bold text-surface-700 mb-3 uppercase tracking-wider">PE Header Info</h4>
                ${peInfo.isValid ? `
                  <div class="space-y-2 text-sm">
                    <div class="flex justify-between"><span class="text-surface-500">Machine:</span> <span>${peInfo.machine}</span></div>
                    <div class="flex justify-between"><span class="text-surface-500">Timestamp:</span> <span class="text-xs">${peInfo.timestamp}</span></div>
                    <div class="flex justify-between"><span class="text-surface-500">Sections:</span> <span>${peInfo.sections}</span></div>
                    <div class="flex justify-between"><span class="text-surface-500">Characteristics:</span> <span class="font-mono">${peInfo.characteristics}</span></div>
                  </div>
                ` : `
                  <div class="flex items-center justify-center h-24 text-surface-400 italic">No valid PE header found</div>
                `}
              </div>
            </div>
