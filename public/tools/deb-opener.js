  ...
  window.initTool = function (toolConfig, mountEl) {
    OmniTool.create(mountEl, toolConfig, {
      ...
      onFile: async function (file, content, h) {
        h.showLoading('Analyzing DEB package...');
        ...
        const txt = new TextDecoder().decode(buffer.slice(0, 64));
        if (txt.startsWith('!<arch>')) {
          debInfo.isValid = true;
          // Basic AR parsing to find members
          const view = new Uint8Array(buffer);
          let offset = 8; // skip !<arch>\n
          while (offset + 60 <= buffer.byteLength && debInfo.contains.length < 5) {
            const name = new TextDecoder().decode(buffer.slice(offset, offset + 16)).trim();
            const size = parseInt(new TextDecoder().decode(buffer.slice(offset + 48, offset + 60)).trim());
            if (isNaN(size)) break;
            debInfo.contains.push(`${name} (${(size / 1024).toFixed(1)} KB)`);
            offset += 60 + size;
            if (offset % 2 !== 0) offset++; // AR alignment
          }
        }
          ...
          <div class="p-6 space-y-6 font-sans">
            <div class="flex items-center justify-between border-b border-surface-200 pb-4">
              <div>
                ...
                <p class="text-sm text-surface-500">${(file.size / 1024).toFixed(2)} KB • Debian Package</p>
              </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <!-- DEB Info Card -->
              <div class="bg-surface-50 rounded-xl p-4 border border-surface-200">
                <h4 class="text-sm font-bold text-surface-700 mb-3 uppercase tracking-wider">Package Contents (AR)</h4>
                ${debInfo.isValid ? `
                  <ul class="text-sm space-y-1 list-disc list-inside text-surface-600">
                    ${debInfo.contains.map(c => `<li>${c}</li>`).join('')}
                    ${debInfo.contains.length === 0 ? '<li class="italic">No members found</li>' : ''}
                  </ul>
                ` : `
                  <div class="flex items-center justify-center h-24 text-surface-400 italic">Not a valid AR/DEB archive</div>
                `}
              </div>
            </div>
