(function() {
  ...
  window.initTool = function(toolConfig, mountEl) {
    OmniTool.create(mountEl, toolConfig, {
      accept: '.iges,.igs',
      dropLabel: 'Drop a .iges file here',
      binary: true,
        ...
        async function parseAndRender() {
          try {
            ...
            if (!result || !result.success || !result.meshes || result.meshes.length === 0) {
              throw new Error('Failed to parse IGES file or file is empty.');
            }

            render3D(result, file, helpers);
          } catch (e) {
            console.error(e);
            helpers.showError('Could not parse IGES file', e.message || 'Unknown error');
          }
        }
      },
      actions: [
  ...
  function render3D(result, file, helpers) {
    ...
    var html = 
      ...
      '<div class="relative w-full h-[600px] bg-slate-900 rounded-xl overflow-hidden shadow-inner border border-surface-200">' +
        '<div id="iges-canvas-mount" class="w-full h-full cursor-move"></div>' +
    ...
    var mountEl = document.getElementById('iges-canvas-mount');
    if (!mountEl) return;
    ...
    if (cameraZ === 0 || isNaN(cameraZ)) cameraZ = 10;
    ...
    // Animation
    function animate() {
      if (!document.getElementById('iges-canvas-mount')) return;
