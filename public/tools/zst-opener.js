/**
 * OmniOpener — Zstandard (ZST) Opener
 * Uses OmniTool SDK and fzstd for client-side decompression.
 */
(function () {
  'use strict';
  ...
  window.initTool = function (toolConfig, mountEl) {
    OmniTool.create(mountEl, toolConfig, {
      binary: true,
      accept: '.zst,.zstd',
      infoHtml: '<strong>Privacy:</strong> Decompression happens entirely in your browser using Zstandard (fzstd). No data is uploaded to any server.',
      ...
      onInit: function (h) {
        h.loadScript('https://cdn.jsdelivr.net/npm/fzstd@0.1.0/umd/index.js');
      },

      onFile: function (file, content, h) {
        if (typeof fzstd === 'undefined') {
          h.showLoading('Loading Zstandard decompressor...');
          setTimeout(() => {
            if (typeof fzstd !== 'undefined') this.onFile(file, content, h);
            else h.showError('Dependency Error', 'Failed to load fzstd library.');
          }, 1000);
          return;
        }

        h.showLoading('Decompressing Zstandard archive...');
        
        // Use a small delay to ensure UI updates
        setTimeout(() => {
          try {
            const uint8 = new Uint8Array(content);
            const decompressed = fzstd.decompress(uint8);
            const originalName = file.name.replace(/\.zst(d)?$/i, '');
              ...
              <div class="p-6 space-y-6 font-sans">
                <div class="flex items-center justify-between border-b border-surface-200 pb-4">
                  <div>
                    ...
                    <p class="text-sm text-surface-500">${(file.size / 1024).toFixed(2)} KB • Zstandard Archive</p>
                  </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div class="bg-surface-50 rounded-xl p-4 border border-surface-200 text-center">
                    <div class="text-[10px] font-bold text-surface-400 uppercase tracking-wider mb-1">Compression Ratio</div>
            ...
            h.showError('Decompression Failed', 'The Zstandard archive might be corrupted or in an unsupported format: ' + err.message);
          }
        }, 50);
      }
    });
