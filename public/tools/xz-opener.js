  ...
  window.initTool = function (toolConfig, mountEl) {
    OmniTool.create(mountEl, toolConfig, {
      ...
      onFile: async function (file, content, h) {
        h.showLoading('Analyzing XZ archive...');
        ...
        if (view.length >= 6) {
          if (view[0] === 0xFD && view[1] === 0x37 && view[2] === 0x7A && view[3] === 0x58 && view[4] === 0x5A && view[5] === 0x00) {
            isXZ = true;
          }
        }
          ...
          <div class="p-6 space-y-6 font-sans">
            <div class="flex items-center justify-between border-b border-surface-200 pb-4">
              <div>
                ...
                <p class="text-sm text-surface-500">${(file.size / 1024).toFixed(2)} KB • XZ Compressed Archive</p>
              </div>
              ...
              <div class="bg-surface-50 rounded-xl p-4 border border-surface-200">
                ...
                ${isXZ ? `
                  <div class="text-sm text-surface-600">
                    <p>Verified <strong>XZ</strong> compression format.</p>
                    <p class="mt-2 text-xs">Signature '\xFD7zXZ\x00' detected at start of file.</p>
                  </div>
                ` : `
                  <div class="text-sm text-surface-400 italic">No XZ signature detected.</div>
                `}
              </div>
            </div>
